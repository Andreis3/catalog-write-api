	...
	Describe("#Validate", func() {
		Context("success cases", func() {
			It("should not return an error when valid entity", func() {
				...
				Expect(entity.GetExternalID()).To(Equal("1"))
				Expect(entity.GetApikey()).To(Equal("apikey"))
				Expect(entity.GetName()).To(Equal("name"))
				Expect(entity.GetDescription()).To(Equal("description"))
				Expect(entity.GetStatus()).To(Equal("enabled"))
				Expect(entity.GetBrand()).To(Equal("brand"))
				Expect(entity.GetReleaseDate()).To(Equal("2021-01-01"))
			})
		})
